# Deploy Cloud TTS Edge Function

## Prerequisites

1. Install Supabase CLI:
```bash
npm install -g supabase
```

2. Login to Supabase:
```bash
supabase login
```

3. Link to your project:
```bash
supabase link --project-ref YOUR_PROJECT_REF
```

## Set Environment Variables

Run these commands to set your Google Cloud credentials as Supabase secrets:

```bash
# Set the credentials from your service account JSON
supabase secrets set GOOGLE_CLIENT_EMAIL="language@gen-lang-client-0018395577.iam.gserviceaccount.com"

supabase secrets set GOOGLE_PROJECT_ID="gen-lang-client-0018395577"

supabase secrets set GOOGLE_PRIVATE_KEY="-----BEGIN PRIVATE KEY-----

JFa8fDcNlnoWgeMFE+RVWno8
-----END PRIVATE KEY-----"
```

**IMPORTANT:** Make sure to keep the quotes and preserve line breaks in the private key!

## Deploy Function

```bash
supabase functions deploy cloud-tts
```

## Test the Function

```bash
curl -X POST 'YOUR_SUPABASE_URL/functions/v1/cloud-tts' \
  -H 'Content-Type: application/json' \
  -d '{
    "text": "你好",
    "languageCode": "cmn-CN",
    "voiceName": "cmn-CN-Wavenet-A"
  }'
```

## Get Function URL

Your function will be available at:
```
https://YOUR_PROJECT_REF.supabase.co/functions/v1/cloud-tts
```

Add this URL to your Vercel environment variables as `SUPABASE_FUNCTIONS_URL` or similar.
